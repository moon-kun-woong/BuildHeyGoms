...
<script>
		function removeClassMember(classId){
			if (confirm("수업을 취소하시겠습니까?")) {
				location.href = "${contextPath}/member/removeClassMemberOnConnectedAsMember?classId=" + classId;
			}
		}
		
		function viewTrainerInfo(trainerId) {
			location.href = "${contextPath}/member/viewTrainerInfo?trainerId=" + trainerId;
		}
...
<!-- Begin -->
	<div class="bg-white p-10 rounded-lg shadow-md mb-4 w-full md:w-3/5">
        <div class="flex justify-between items-center mb-4">
            <div>
                <h2 class="text-xl font-semibold">나의 트레이너</h2>
	        ...
	        <div class="trainer-section">
				...
				<c:choose>
				    <c:when test="${not empty classDTO}">
			            <c:forEach var="classDTO" items="${classDTO}">
			                <div class="trainer-card" onclick="redirectToTrainerDetail('${classDTO.classId}')">
			                    <img src="https://source.unsplash.com/random/300x200?fitness" alt="Trainer">
			                    <div class="trainer-info">
			                        <strong>${classDTO.trainerId} 트레이너</strong>
			                        <p>강의 날짜: ${classDTO.selectedDate}</p>
			                        <p class="bg-gray-100">BuildHeyGo 인증 완료</p>
			                        <p>지역: ${classDTO.classLocation}</p>
			                        <p>강의 내용: ${classDTO.classContent}</p>
			                        <p hidden="${classDTO.classId }" id="trainerId"/> 
			                    </div>
			                    <div>
			                        ...
			                        <button class="button-secondary button-info" onclick="viewTrainerInfo('${classDTO.trainerId}');">트레이너 정보</button>
			                    </div>
			                </div>
			            </c:forEach>
				    </c:when>
